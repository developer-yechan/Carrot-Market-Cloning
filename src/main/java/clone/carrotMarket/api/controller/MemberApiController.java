...
public class MemberApiController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "회원가입 성공", content = @Content(schema = @Schema(implementation = SuccessDTO.class))),
            @ApiResponse(responseCode = "400", description = "BAD REQUEST", content = @Content(schema = @Schema(implementation = ErrorDTO.class))),
    })
    @PostMapping("/signup")
    public SuccessDTO saveMember(@Valid @RequestBody CreateMemberDto createMemberDto){
        ...
        return new SuccessDTO(201,memberId.toString());
    }
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "사용자 정보 수정 성공", content = @Content(schema = @Schema(implementation = SuccessDTO.class))),
            @ApiResponse(responseCode = "400", description = "BAD REQUEST", content = @Content(schema = @Schema(implementation = ErrorDTO.class))),
    })
    @PutMapping
    public SuccessDTO editMember(@Valid @RequestBody EditMemberDto editMemberDto) throws IOException {
            memberService.editMember(editMemberDto);
            return new SuccessDTO(200,"사용자 정보 수정 성공");
    }

    @Operation(summary = "프로필 사진 삭제", description = "프로필 사진을 삭제한다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "프로필 사진 삭제 완료", content = @Content(schema = @Schema(implementation = SuccessDTO.class))),
            @ApiResponse(responseCode = "400", description = "BAD REQUEST", content = @Content(schema = @Schema(implementation = ErrorDTO.class))),
    })
    @Parameters({
    ...
    public SuccessDTO deleteProfileImage(@PathVariable Long memberId){
        memberService.deleteProfileImage(memberId);
        return new SuccessDTO(200,"프로필 사진 삭제 완료");
    }
}
